...
phase(0), current_query(Q),
fact([edge,X,Y],O1,Q1,M1), fact([path,Y,Z],O2,Q2,_M2) \ apply_one, fact([path,X,Z],add,_,_) <=> 
	member([del,Q],[[O1,Q1],[O2,Q2]]) |
	compute_positive_mark([(O1,M1,ex), (O2,_M2,im)],M),
...
fact([edge,X,Y],add,Q,M1) \ apply_one, fact([path,X,Y],del,_,_) <=> 
	...
	% edge(X,Y), path(Y,Z) --> path(X,Z)
phase(1),
fact([edge,X,Y],add,Q,M1), fact([path,Y,Z],add,_,_M2) \ apply_one, fact([path,X,Z],del,_,_) <=>
	compute_negative_mark([M1, _M2], M),
...
fact([edge,X,Y],add,Q,M1) ==> 
	...
	% edge(X,Y), path(Y,Z) --> path(X,Z)
phase(3), current_query(Q),
fact([edge,X,Y],add,Q1,M1), fact([path,Y,Z],add,Q2,_M2) ==>
	member(Q, [Q1, Q2]) |
	compute_negative_mark([M1, _M2], M),
