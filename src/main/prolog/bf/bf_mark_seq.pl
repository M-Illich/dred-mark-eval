...
% -- backward chaining --
fact([edge1,X,Y],chk,_,_) \ fact([edge,X,Y],add,Q,M) <=>
	fact([edge,X,Y],prv,Q,M),
	applied_rules(1,bwd).	
fact([edge2,X,Y],chk,_,_) \ fact([edge1,X,Y],add,Q,M) <=>
	fact([edge1,X,Y],chk,Q,M),
	applied_rules(1,bwd).		
fact([edge3,X,Y],chk,_,_) \ fact([edge2,X,Y],add,Q,M) <=>
	fact([edge1,X,Y],chk,Q,M),
	applied_rules(1,bwd).		
fact([edge4,X,Y],chk,_,_) \ fact([edge3,X,Y],add,Q,M) <=>
	fact([edge1,X,Y],chk,Q,M),
	applied_rules(1,bwd).		
...
fact([edge,X,Y],prv,_,M) \ fact([edge1,X,Y],chk,Q,_) <=>
	fact([edge1,X,Y],prv,Q,M),
	applied_rules(1,fwd).	
...
phase(1), fact([edge,X,Y],prv,_,M) \ fact([edge1,X,Y],add,Q,_) <=>
	fact([edge1,X,Y],dly,Q,M),
	applied_rules(1,fwd).	
...
phase(4) \ fact([edge1,X,Y],add,Q,1) <=> 
	fact([edge1,X,Y],chk0,Q,_),
	marked_facts(1,add,[edge1,X,Y]).
