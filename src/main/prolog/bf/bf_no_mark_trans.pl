...
% no processing needed if fact already proven
...
% -- backward chaining --
fact([path,X,Y],chk,_,_) \ fact([edge,X,Y],add,Q,_) <=>
	fact([edge,X,Y],prv,Q,_),
	applied_rules(1,bwd).	
fact([path,X,Z],chk,_,_) \ fact([edge,X,Y],add,Q1,_), fact([path,Y,Z],add,Q2,_) <=>
	fact([edge,X,Y],prv,Q1,_),
	fact([path,Y,Z],chk,Q2,_),
	applied_rules(1,bwd).		
fact([path,X,Z],chk,_,_), fact([edge,X,Y],prv,_,_) \ fact([path,Y,Z],add,Q2,_) <=>
	fact([path,Y,Z],chk,Q2,_),
	applied_rules(1,bwd).		
fact([path,X,Z],chk,_,_), fact([path,Y,Z],prv,_,_) \ fact([edge,X,Y],add,Q1,_) <=>
	fact([edge,X,Y],prv,Q1,_),
	applied_rules(1,bwd).	
fact([path,X,Z],chk,_,_) \ fact([edge,X,Y],add,Q1,_), fact([path,Y,Z],dly,Q2,_) <=>
	fact([edge,X,Y],prv,Q1,_),
	fact([path,Y,Z],prv,Q2,_),
	applied_rules(1,bwd).	
fact([path,X,Z],chk,_,_),  fact([edge,X,Y],prv,_,_) \ fact([path,Y,Z],dly,Q2,_) <=>
	fact([path,Y,Z],prv,Q2,_),
	applied_rules(1,bwd).	
...
fact([edge,X,Y],prv,_,_) \ fact([path,X,Y],chk,Q,_) <=> 
	fact([path,X,Y],prv,Q,_),
	applied_rules(1,fwd).
fact([edge,X,Y],prv,_,_), fact([path,Y,Z],prv,_,_) \ fact([path,X,Z],chk,Q1,_) <=>
	fact([path,X,Z],prv,Q1,_),
	applied_rules(1,fwd).		
% not-yet-checked fact is delayed	
phase(1), fact([edge,X,Y],prv,_,_) \ fact([path,X,Y],add,Q,_) <=> 
	fact([path,X,Y],dly,Q,_),
	applied_rules(1,fwd).
phase(1), fact([edge,X,Y],prv,_,_), fact([path,Y,Z],prv,_,_) \ fact([path,X,Z],add,Q1,_) <=>
	fact([path,X,Z],dly,Q1,_),
	applied_rules(1,fwd).	
...
fact([edge,X,Y],add,Q,_) ==> 
	derived_fact([path,X,Y],Q,_).
	
	% edge(X,Y), path(Y,Z) --> path(X,Z)
phase(3), current_query(Q),
fact([edge,X,Y],add,Q1,_), fact([path,Y,Z],add,Q2,_) ==>
	member(Q, [Q1, Q2]) |
	derived_fact([path,X,Z],Q,_).
