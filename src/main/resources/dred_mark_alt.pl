...
fact([edge,X,Y],del,Q,M) \ apply_one, fact([edge1,X,Y],add,_,_) <=> 
	...
	% edge1(X,Y) --> edge2(X,Y)
phase(0), current_query(Q),
fact([edge1,X,Y],del,Q,M) \ apply_one, fact([edge2,X,Y],add,_,_) <=> 
	...
	% edge2(X,Y) --> edge3(X,Y)
phase(0), current_query(Q),
fact([edge2,X,Y],del,Q,M) \ apply_one, fact([edge3,X,Y],add,_,_) <=> 
...
% look for a rule instance that can still derive a deleted fact
	...
	% edge(X,Y) --> edge1(X,Y)
phase(1),
fact([edge,X,Y],add,Q,M) \ apply_one, fact([edge1,X,Y],del,_,_) <=> 
	...
	% edge1(X,Y) --> edge2(X,Y)
phase(1),
fact([edge1,X,Y],add,Q,M) \ apply_one, fact([edge2,X,Y],del,_,_) <=> 
	...
	% edge2(X,Y) --> edge3(X,Y)
phase(1),
fact([edge2,X,Y],add,Q,M) \ apply_one, fact([edge3,X,Y],del,_,_) <=> 
	...
	% edge3(X,Y) --> edge4(X,Y)
phase(1),
fact([edge3,X,Y],add,Q,M) \ apply_one, fact([edge4,X,Y],del,_,_) <=> 
...
fact([edge,X,Y],add,Q,M) ==> derived_fact([edge1,X,Y],Q,M).
	
	% edge1(X,Y) --> edge2(X,Y)
phase(3), current_query(Q),
fact([edge1,X,Y],add,Q,M) ==> derived_fact([edge2,X,Y],Q,M).
	
	% edge2(X,Y) --> edge3(X,Y)
phase(3), current_query(Q),
fact([edge2,X,Y],add,Q,M) ==> derived_fact([edge3,X,Y],Q,M).
	
	% edge3(X,Y) --> edge4(X,Y)
phase(3), current_query(Q),
fact([edge3,X,Y],add,Q,M) ==> derived_fact([edge4,X,Y],Q,M).	
