...
query(_,Q), mark_query(Q), fact(F,add,_,M) \ pending_fact(F,del,Q) <=>
...
fact([edge,X,Y],del,Q,_) \ apply_one, fact([path,X,Y],add,_,_) <=> 
	fact([path,X,Y],del,Q,_),
	% enable counting of applied rules per phase
	applied_rules(1,del).	
	
	% edge(X,Y), path(Y,Z) --> path(X,Z)
phase(0), current_query(Q),
fact([edge,X,Y],O1,Q1,_), fact([path,Y,Z],O2,Q2,_) \ apply_one, fact([path,X,Z],add,_,_) <=> 
	member([del,Q],[[O1,Q1],[O2,Q2]]) |
	fact([path,X,Z],del,Q,_),
	% enable counting of applied rules per phase
	applied_rules(1,del).		
