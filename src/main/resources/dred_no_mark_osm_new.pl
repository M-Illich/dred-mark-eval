...
:- chr_constraint init/1, stream/1,
	loop/0, read_stream/1, apply_one/0, phase/1, 
	available_input/2, extract_input/2,
...
extract_input("stream_end",_) <=>	
	stream_end.
% input indicates end of update	
extract_input("update_end",_) <=>	
	% insert query asking for every fact
	create_query(_Q).
% input indicates end of update's add-part	
extract_input("delete",_) <=>	
	read_stream(del).

% -- input is a fact --
next_query_id(Q) \ extract_input(X,O) <=>	
	term_string(F,X),  
	pending_fact(F,O,Q), 
	read_stream(O).
