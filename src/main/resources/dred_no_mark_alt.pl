...
fact([edge,X,Y],del,Q) \ apply_one, fact([edge1,X,Y],add,_) <=> 
	fact([edge1,X,Y],del,Q),
	% enable counting of applied rules per phase
	applied_rules(1,del).	
	
	% edge1(X,Y) --> edge2(X,Y)
phase(0), current_query(Q),
fact([edge1,X,Y],del,Q) \ apply_one, fact([edge2,X,Y],add,_) <=> 
	fact([edge2,X,Y],del,Q),
	% enable counting of applied rules per phase
	applied_rules(1,del).	
	
	% edge2(X,Y) --> edge3(X,Y)
phase(0), current_query(Q),
fact([edge2,X,Y],del,Q) \ apply_one, fact([edge3,X,Y],add,_) <=> 
	fact([edge3,X,Y],del,Q),
	% enable counting of applied rules per phase
	applied_rules(1,del).
	
	% edge3(X,Y) --> edge4(X,Y)
phase(0), current_query(Q),
fact([edge3,X,Y],del,Q) \ apply_one, fact([edge4,X,Y],add,_) <=> 
	fact([edge4,X,Y],del,Q),
	% enable counting of applied rules per phase
	applied_rules(1,del).			
...
% look for a rule instance that can still derive a deleted fact
	...
	% edge(X,Y) --> edge1(X,Y)
phase(1),
fact([edge,X,Y],add,Q) \ apply_one, fact([edge1,X,Y],del,_) <=> 
	fact([edge1,X,Y],add,Q),
	% enable counting of applied rules per phase
	applied_rules(1,red).		
	
	% edge1(X,Y) --> edge2(X,Y)
phase(1),
fact([edge1,X,Y],add,Q) \ apply_one, fact([edge2,X,Y],del,_) <=> 
	fact([edge2,X,Y],add,Q),
	% enable counting of applied rules per phase
	applied_rules(1,red).
	
	% edge2(X,Y) --> edge3(X,Y)
phase(1),
fact([edge2,X,Y],add,Q) \ apply_one, fact([edge3,X,Y],del,_) <=> 
	fact([edge3,X,Y],add,Q),
	% enable counting of applied rules per phase
	applied_rules(1,red).
	
	% edge3(X,Y) --> edge4(X,Y)
phase(1),
fact([edge3,X,Y],add,Q) \ apply_one, fact([edge4,X,Y],del,_) <=> 
	fact([edge4,X,Y],add,Q),
	% enable counting of applied rules per phase
	applied_rules(1,red).		
...
fact([edge,X,Y],add,Q) ==> derived_fact([edge1,X,Y],Q).
	
	% edge1(X,Y) --> edge2(X,Y)
phase(3), current_query(Q),
fact([edge1,X,Y],add,Q) ==> derived_fact([edge2,X,Y],Q).
	
	% edge2(X,Y) --> edge3(X,Y)
phase(3), current_query(Q),
fact([edge2,X,Y],add,Q) ==> derived_fact([edge3,X,Y],Q).
	
	% edge3(X,Y) --> edge4(X,Y)
phase(3), current_query(Q),
fact([edge3,X,Y],add,Q) ==> derived_fact([edge4,X,Y],Q).
