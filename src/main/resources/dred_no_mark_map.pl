...
phase(0), current_query(Q),
nextInWay(_,X,Z1,O1,Q1), nextInWay(X,_,Z2,O2,Q2) 
\ apply_one, connection(Z1,Z2,add,_)<=> 
	Z1 \== Z2,
	member([del,Q],[[O1,Q1],[O2,Q2]]) | 
	connection(Z1,Z2,del,Q),
...
% phase(0), current_query(Q),
% connection(X,Y,del,Q)
% \ apply_one, connection(Y,X,add,_)<=> 
	% connection(Y,X,del,Q),
	% applied_rules(1,del).		
...
phase(1), 
nextInWay(_,X,Z1,add,Q), nextInWay(X,_,Z2,add,_) 
\ apply_one, connection(Z1,Z2,del,_)<=> 
	Z1 \== Z2 | 
	connection(Z1,Z2,add,Q),
	applied_rules(1,red).		
...
% \ apply_one, connection(Y,X,del,_)<=> 
	% connection(Y,X,add,Q),
	% applied_rules(1,red).		
...
phase(3), current_query(Q),
nextInWay(_,X,Z1,add,Q1), nextInWay(X,_,Z2,add,Q2) ==>
	Z1 \== Z2,
	member(Q,[Q1,Q2]) | 
	derived_fact([connection,Z1,Z2],Q).	
